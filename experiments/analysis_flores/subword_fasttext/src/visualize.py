"""
2D projections of varieties (MDS and t-SNE) for visualisation.

Shared with the TF-IDF baseline's visualise module in spirit: we always
feed a *precomputed* cosine distance matrix, so there is no difference
in behaviour between subword pipelines (fasttext / bpe).

- MDS preserves global variance; relative distances are faithful.
- t-SNE emphasises local clusters; good for perceptual groupings but
...
def project_tsne(dist: np.ndarray, perplexity: float = 4.0, random_state: int = 42) -> np.ndarray:
    """t-SNE with precomputed distances; low perplexity because n=16."""
    tsne = TSNE(
        n_components=2,
        metric="precomputed",
        init="random",
...
def plot_projection(
    ...
    """Scatter plot coloured by family, with full English names as labels."""
    colors = _colors_for(codes)
    labels = [VARIETY_NAMES[c] for c in codes]

    fig, ax = plt.subplots(figsize=(11.0, 6.8))
    ax.scatter(coords[:, 0], coords[:, 1], c=colors, s=130,
...
def visualize_pipeline(
    ...
    """
    Produce MDS + t-SNE plots for one pipeline, saved to
    ...
    plot_projection(
        coords_mds, codes,
        title=f"MDS projection ({pipeline}, cosine distance)",
        out_path=path_mds,
    )
    ...
    plot_projection(
        coords_tsne, codes,
        title=f"t-SNE projection ({pipeline}, cosine, perplexity=4)",
        out_path=path_tsne,
    )
