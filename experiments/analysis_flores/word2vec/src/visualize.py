"""
2D projections of varieties (MDS and t-SNE) for Word2Vec.

We always feed a precomputed cosine distance matrix, so the
projections are comparable across approaches (TF-IDF, subword,
Word2Vec, multilingual).
"""
...
def visualize_pipeline(
    ...
    plot_projection(
        coords_mds, codes,
        title="MDS projection (Word2Vec, cosine distance)",
        out_path=path_mds,
    )
    ...
    plot_projection(
        coords_tsne, codes,
        title="t-SNE projection (Word2Vec, cosine, perplexity=4)",
        out_path=path_tsne,
    )
