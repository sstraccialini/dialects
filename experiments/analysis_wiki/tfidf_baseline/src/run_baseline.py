"""
End-to-end orchestrator for the TF-IDF baseline.

Launch with:
    python tfidf_baseline/run_baseline.py

Options:
    --sample-size N    sentences per variety (default from config.SAMPLE_SIZE)
    --pipeline {word,char,both}  which pipeline(s) to run (default both)
    --random-state N   random seed (default from config.RANDOM_STATE)

Outputs in `tfidf_baseline/results/`:
...
def run_pipeline(pipeline_name: str, fit_fn, variety_docs, codes) -> dict:
    ...
    # Persist distances, top features, nearest neighbors
...
def main():
    parser = argparse.ArgumentParser(description="TF-IDF baseline orchestrator")
    ...
    # Resolve defaults from config here (not in argparse defaults) so that
    # changes to config take effect without editing argparse as well.
    ...
    # Aggregate silhouette report in shared/
    raw = [
        {
            "pipeline": r["pipeline"],
