...
Inputs: a n x n cosine distance matrix and the ordered list of codes.
We feed the precomputed cosine distance matrix directly
(metric='precomputed'). With only 16 points this is robust and fast.
"""
...
def visualize_pipeline(
    ...
    plot_projection(
        coords_mds, codes,
        title=f"MDS projection ({pipeline} embeddings, cosine distance)",
        out_path=path_mds,
    )
    ...
    plot_projection(
        coords_tsne, codes,
        title=f"t-SNE projection ({pipeline} embeddings, cosine, perplexity=4)",
        out_path=path_tsne,
    )
