"""
Hierarchical clustering + silhouette score for the multilingual approach.

Same semantics as the other analysis_flores modules (TF-IDF, subword, Word2Vec):
    - average linkage (coherent with cosine distance; we deliberately
      avoid Ward, which requires Euclidean distances)
...
def silhouette_report(dist: np.ndarray, codes: List[str]) -> dict:
...
def hierarchical_linkage(dist: np.ndarray, method: str = LINKAGE_METHOD) -> np.ndarray:
...
def cluster_pipeline(
    ...
    """
    End-to-end wrapper: silhouette + linkage + dendrogram.

    `model_name` only tweaks the plot title.
    `pipeline` is the results subdir ("" = root results dir).
    """
    ...
    title = (f"Hierarchical clustering ({model_name}, "
             f"cosine + {LINKAGE_METHOD} linkage)")
    plot_dendrogram(Z, codes, title=title, out_path=dendro_path)
    report["dendrogram_path"] = str(dendro_path)
    report["pipeline"] = pipeline or "multilingual"
    report["model_name"] = model_name
    return report


def save_silhouette_report(reports: List[dict]) -> Path:
    lines = ["Silhouette report (multilingual)", "=" * 60, ""]
    for r in reports:
        lines.append(f"Pipeline: {r['pipeline']}  ({r.get('model_name', '')})")
    ...
    out = results_subdir("") / "silhouette_report.txt"
