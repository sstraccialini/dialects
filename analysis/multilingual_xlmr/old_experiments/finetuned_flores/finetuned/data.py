"""
Fine-tuning–specific data loaders.

  - load_wiki_texts_for_mlm   flat list of Wiki sentences for MLM
  - load_oldi_pairs / load_all_oldi_pairs   Italian ↔ dialect pairs for TLM
...
def load_wiki_texts_for_mlm(
    ...
    """Flat list of dialect Wiki sentences for MLM continued pretraining.

    Default codes: 6 Italo-Romance dialects + Italian.
    """
    if codes is None:
...
def load_oldi_pairs(code: str) -> List[Tuple[str, str]]:
    """Load (italian, dialect) parallel pairs for one Romance dialect."""
