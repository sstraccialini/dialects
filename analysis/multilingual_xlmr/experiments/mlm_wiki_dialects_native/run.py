"""
ABLATION CELL — XLM-R + NATIVE text + dialect-only training.
...
Uses NATIVE path aliases from analysis.multilingual_xlmr.core.config.

Launch (HPC):
    sbatch slurm/jobs/multilingual_xlmr__mlm_wiki_dialects_native.slurm
"""
...
def evaluate_on(test_name: str, test_data, embedder, codes):
    ...
    centroid_report = variety_eval(
        ...
        method_label=f"XLM-R adapted ({test_name.upper()} centroid, dialects-native)",
    )
    ...
    parallel_eval(
        ...
        method_label=f"XLM-R adapted ({test_name.upper()} parallel, dialects-native)",
    )
