"""
XLM-R — continued MLM on Wiki+OLDI dialect-only (NATIVE), eval on FLORES.

Cell 11 of the FINAL 12-cell experimental matrix (EXPERIMENTAL_PLAN.md §3.3).

Setup:
  base model    : xlm-roberta-base
  loss          : MLM (continued pretraining)
  text variant  : NATIVE (matches XLM-R tokenizer pretraining)
  training data : Wiki + OLDI dialect (cap rule §3.4) — DIALECTS ONLY (6 codes)
  hyperparams   : 3 epochs, batch 16 × grad_accum 4, lr 3e-5, max_length 512
  eval          : centroid + parallel on FLORES — ALL 17 varieties

Rationale: training only on dialects preserves the cross-language structure
of the standard languages (no rehearsal → no catastrophic forgetting on
non-target families).

Launch:
    python analysis/multilingual_xlmr/experiments/xlmr_finetuned_wikiOLDI_dialects_native/run.py
"""
...
EXPERIMENT = "xlmr_finetuned_wikiOLDI_dialects_native"
TEXT_VARIANT = "native"
...
def evaluate_on(test_name: str, test_data, embedder, codes):
    ...
    rep = variety_eval(X, codes_out, out_dir=er_centroid,
                       method_label=f"XLM-R FT ({EXPERIMENT}, {test_name} centroid)")
    ...
    parallel_eval(per_variety, out_dir=er_parallel,
                  method_label=f"XLM-R FT ({EXPERIMENT}, {test_name} parallel)")
...
def main():
    parser = argparse.ArgumentParser(description="XLM-R MLM finetune on Wiki+OLDI dialects → FLORES (native)")
    ...
    model_dir = mo_root / "models" / "xlmr_finetuned"
