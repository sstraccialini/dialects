"""
XLM-R base — ZERO-SHOT (no fine-tuning), evaluate on FLORES (NATIVE).

Cell 8 of the FINAL 12-cell experimental matrix (EXPERIMENTAL_PLAN.md §3.2).

Setup:
  model       : xlm-roberta-base (HF)
  fine-tuning : NONE — straight encoder forward pass
  text        : NATIVE (matches XLM-R pretraining tokenizer)
  eval        : centroid + parallel on FLORES (1827 × 17)

Launch:
    python analysis/multilingual_xlmr/experiments/xlmr_zeroshot_native/run.py
"""
...
EXPERIMENT = "xlmr_zeroshot_native"
TEXT_VARIANT = "native"
...
def evaluate_on(test_name: str, test_data, embedder: MultilingualEmbedder, codes):
    ...
    rep = variety_eval(X, codes_out, out_dir=er_centroid,
                       method_label=f"XLM-R zero-shot ({test_name} centroid)")
    ...
    parallel_eval(per_variety, out_dir=er_parallel,
                  method_label=f"XLM-R zero-shot ({test_name} parallel)")
...
def main():
    parser = argparse.ArgumentParser(description="XLM-R zero-shot → FLORES (native)")
    ...
    embedder = MultilingualEmbedder(
        model_name=MODEL_NAME, device=args.device, max_length=MAX_LENGTH,
    )
