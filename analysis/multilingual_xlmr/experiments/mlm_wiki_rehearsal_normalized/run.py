"""
ABLATION CELL — XLM-R + NORMALIZED text + 13-variety rehearsal training.

Purpose: complete the 2x2 ablation matrix (text variant × training regime)
for XLM-R, alongside:
  - mlm_wiki_to_flores_oldi/       (native + 13 var, +0.062 sil_rom OLDI) done
  - mlm_wiki_dialects_native/      (native + 6 dial)                       todo
  - this run                       (norm + 13 var)                          ←
  - (norm + 6 dial — handled by older run, sil_rom OLDI = +0.121)
...
Bypasses xlmr.core.config aliases (which point to NATIVE) and uses the
NORMALIZED paths from analysis._shared.varieties directly.

Launch (HPC):
    sbatch slurm/jobs/multilingual_xlmr__mlm_wiki_rehearsal_normalized.slurm
"""
...
def evaluate_on(test_name: str, test_data, embedder, codes):
    ...
    centroid_report = variety_eval(
        ...
        method_label=f"XLM-R adapted ({test_name.upper()} centroid, rehearsal-norm)",
    )
    ...
    parallel_eval(
        ...
        method_label=f"XLM-R adapted ({test_name.upper()} parallel, rehearsal-norm)",
    )
