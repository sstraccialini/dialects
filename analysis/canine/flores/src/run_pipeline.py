"""
End-to-end orchestrator for the CANINE (char-level) embeddings approach
on FLORES+.

Launch (from repo root, with venv active):
    python analysis/canine/flores/src/run_pipeline.py

Method outputs (sentence/variety vectors, run stats) land under
``method_outputs/``; standard evaluation artefacts (distances, dendrogram,
projections, silhouette) under ``evaluation_results/``.
"""
...
from embedder import CanineEmbedder
...
def main():
    parser = argparse.ArgumentParser(
        description="CANINE (char-level) embeddings orchestrator (FLORES+)"
    )
    ...
    print("CANINE embeddings on FLORES+")
    print("=" * 60)
    print(f"  model_name    = {args.model_name}")
    print(f"  sample_size   = {sample_size}")
    print(f"  random_state  = {random_state}")
    print(f"  batch_size    = {args.batch_size}")
    print(f"  max_length    = {MAX_LENGTH}  (characters)")
    ...
    report = run_evaluation(
        ...
        method_label=f"CANINE OTS ({args.model_name})",
    ...
    print(f"  canine ({args.model_name}): X.shape={X.shape}  "
