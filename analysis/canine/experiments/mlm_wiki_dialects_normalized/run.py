"""
CANINE: continued MLM pretraining on Wiki (only the 6 dialects), text
**NORMALIZED** (lowercase ASCII), then evaluation on FLORES + OLDI.

Ablation experiment to disentangle the regression observed under the
rehearsal-style native setup. Reproduces the OLD-style training:
  - text variant: normalized (lowercase ASCII, no diacritics)
  - training set: only fur, lij, lmo, sc, scn, vec
  - evaluation:    all 13 varieties (centroid + parallel) on FLORES + OLDI

Output goes to its own folder (mlm_wiki_dialects_normalized/), so the
existing rehearsal-13 / native results are preserved.

Launch (HPC):
    sbatch slurm/jobs/canine__mlm_wiki_dialects_normalized.slurm
"""
...
# Use NORMALIZED paths (the originals from analysis._shared.varieties),
# bypassing the canine.core.config aliases that point to NATIVE.
...
METHOD = "canine"
EXPERIMENT = "mlm_wiki_dialects_normalized"
...
def evaluate_on(test_name: str, test_data, embedder, codes):
    ...
    centroid_report = variety_eval(
        ...
        method_label=f"CANINE adapted ({test_name.upper()} centroid, dialects-norm)",
    )
    ...
    parallel_eval(
        ...
        method_label=f"CANINE adapted ({test_name.upper()} parallel, dialects-norm)",
    )
...
def main():
    ...
    mo_root.mkdir(parents=True, exist_ok=True)
    model_dir = mo_root / "models" / "mlm_wiki_dialects_norm"

    # Step 1: Wiki training (NORMALIZED, only 6 dialects)
    ...
    # Step 3: evaluation on NORMALIZED FLORES + OLDI (all 13)
