"""
FastText subword — train Wiki+OLDI dialect, evaluate on FLORES (NORMALIZED).

Cell 3 of the FINAL 12-cell experimental matrix (EXPERIMENTAL_PLAN.md §3.1).

Setup:
  train  : Wiki + OLDI dialect (cap rule §3.4) — text from cleaned_normalized
  embed  : mean-pool token vectors per FLORES sentence, then mean per variety
  eval   : centroid + parallel alignment on FLORES (1827 parallel sentences)

Launch:
    python analysis/fasttext/experiments/fasttext_wikiOLDI_normalized/run.py
"""
...
METHOD = "fasttext"
EXPERIMENT = "fasttext_wikiOLDI_normalized"
...
def main():
    parser = argparse.ArgumentParser(description="FastText wikiOLDI → FLORES (normalized)")
