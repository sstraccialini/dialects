"""
End-to-end orchestrator for the Subword / FastText approach on Wikipedia
data.
...
Launch (from repo root, with venv active):
    python analysis/fasttext/wiki/src/run_approach.py
"""
...
def run_fasttext_step(data, codes) -> dict:
    print("\n=== Pipeline: fasttext ===")
...
def main():
    parser = argparse.ArgumentParser(
        description="Subword / FastText approach orchestrator (Wiki)"
    )
    ...
    print("Subword / FastText approach (Wiki)")
    print("=" * 60)
    ...
    if args.pipeline in ("fasttext", "both"):
        reports.append(run_fasttext_step(data, VARIETY_CODES))
    if args.pipeline in ("bpe", "both"):
        reports.append(run_bpe_step(data, VARIETY_CODES))
    ...
    for r in reports:
        ...
        print(f"  {r['variant']:>9}: shape={r['shape']}  "
