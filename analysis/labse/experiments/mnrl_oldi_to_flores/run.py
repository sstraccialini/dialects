"""
LaBSE: MNRL (contrastive) on OLDI parallel pairs only, then eval on FLORES+.

Counter-example experiment for the paper: LaBSE was originally trained
with translation-ranking objectives (AMS / contrastive). When we fine-tune
it with MNRL on (Italian, dialect) parallel pairs, the model learns to
collapse translations into the same embedding — which DESTROYS the
language-identity signal we need to measure typological similarity.

Expected outcome: silhouette_romance drops dramatically vs zero-shot
LaBSE baseline. This is the methodological lesson for the paper:
contrastive task fine-tuning is the WRONG approach for cross-language
similarity measurement.

We reuse `analysis.sentence_minilm.core.{data_loader,embedder,trainer,evaluate}`
because they are model-agnostic (the trainer/embedder accept any
sentence-transformers model via the `base_model` / `model_name_or_path`
parameter).

Launch (HPC, requires GPU):
    sbatch slurm/jobs/labse__mnrl_oldi_to_flores.slurm
"""
...
# LaBSE: 768-dim sentence encoder pretrained on translation pairs (109 langs).
...
def main():
    ...
    mo_root.mkdir(parents=True, exist_ok=True)
    model_dir = mo_root / "models" / "labse_mnrl_oldi"

    print("Loading OLDI pairs (italian <-> dialect, 6 dialects) ...")
    pairs = load_all_oldi_pairs()
    print(f"  total OLDI pairs: {len(pairs):,}")

    pd.DataFrame({"n_pairs": [len(pairs)]}).to_csv(
        mo_root / "run_stats.csv", index=False
    )
