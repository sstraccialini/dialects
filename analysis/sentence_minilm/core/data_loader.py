"""
Data loaders for the sentence-baseline experiments.

Adds an article-aware loader for Wiki because the multi-stage aggregation
(sentence → article → variety) needs the `article_id` column.
"""
...
def load_wiki_with_article_ids(
    codes: List[str] = None,
    sample_size: int = SAMPLE_SIZE,
    random_state: int = RANDOM_STATE,
    verbose: bool = True,
    ...
    """
    Same as load_wiki_for_training but preserves `article_id` in DataFrames.
    Used by the article-aware Wiki experiments.
    """
    if codes is None:
        codes = VARIETY_CODES
    ...
    for code in codes:
        path = _wiki_csv_path(code)
        df = (pd.read_csv(path, usecols=["text", "article_id"])
                .dropna(subset=["text"])
        ...
        if verbose:
            print(f"  [wiki   {code:>4}] available={n_available:>7d}  "
...
# --------------------------------------------------------------------------- #
# OLDI parallel pairs Italian↔dialect — for contrastive (MNRL) training
# --------------------------------------------------------------------------- #
