...
1. Load per-variety CSVs from `Dataset/wiki/{code}.csv` preserving the
...
```
analysis/sentence_baseline/wiki/
├── README.md
├── requirements.txt
├── results/
│   ├── sentence/
...
```bash
cd analysis/sentence_baseline/wiki/src
python run_sentence_baseline.py
```

Outputs land in `analysis/sentence_baseline/wiki/results/sentence/`
