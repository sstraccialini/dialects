"""
End-to-end orchestrator for the *adapted* multilingual XLM-R approach on
FLORES+ (continued pretraining or contrastive variant).

Launch (from repo root, with venv active):
    python analysis/multilingual_xlmr_adapted/flores/src/run_pipeline.py
"""
...
def aggregate_variety_vectors(sentence_vectors, sentence_codes, codes=VARIETY_CODES):
    codes_ordered = []
    rows = []
...
def main():
    parser = argparse.ArgumentParser(
        description="Multilingual XLM-R (adapted) orchestrator (FLORES+)"
    )
    ...
    print("Multilingual (adapted) embeddings on FLORES+")
    print("=" * 60)
    ...
    report = run_evaluation(
        ...
        method_label=f"XLM-R adapted ({args.model_name})",
    ...
    print(f"  multilingual_xlmr_adapted ({args.model_name}): X.shape={X.shape}  "
