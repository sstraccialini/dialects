"""
Load FLORES+ per-variety .txt files for the multilingual approach.

Same contract as the other dataset loaders: each file
`Dataset/flores/flores_plus/<slug>.txt` contains 2009 sentences (one per
line). With FLORES+ the default SAMPLE_SIZE just loads everything.

The multilingual embedder consumes raw sentence strings (no
preprocessing), because the transformer tokenizer handles
...
def load_all_varieties(
    ...
    Returns:
        data:  {slug: list[str]} of sampled sentences (raw text).
...
def iter_labeled_sentences(
    ...
    (sentences, variety_codes), preserving canonical variety order.
    """
    sents: List[str] = []
    slugs: List[str] = []
    for slug in codes:
