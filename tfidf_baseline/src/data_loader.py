...
CSVs in `datasets/{code}.csv` have columns:
...
Key choices:
- Sub-sampling: if a variety has fewer than SAMPLE_SIZE sentences
  (e.g. ar ~16k), we take all of them. Otherwise we sample without
  replacement with a fixed RANDOM_STATE.
- For robustness we return both a DataFrame (handy for exploration)
  and a dict {code: list[str]} (used downstream for vectorization).
"""
...
def load_variety_sentences(
    ...
    # Only read the 'text' column to save RAM; label/article_id are not
    # needed for the baseline.
    df = pd.read_csv(path, usecols=["text"])
    # Sanity check: drop any residual NaN from generation.
    ...
    if n <= sample_size:
        ...
        sampled = (
            df.sample(n=sample_size, random_state=random_state)["text"].tolist()
        )
