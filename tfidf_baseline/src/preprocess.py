...
- Mask numbers: any digit sequence becomes ' NUM ' (spaces on both sides
  so we don't create spurious char n-grams).

Implementation: pure-Python regex, applicable to both a list of
sentences (output of data_loader) and an already-aggregated
super-document.
"""
...
# \d+ matches any sequence of arabic digits. Native Arabic/Greek numerals
# are rarely used in Wikipedia (usually romanized); if needed we could
# extend with \p{N} via the `regex` package (not added to avoid an
# extra dependency).
_NUMBER_RE = re.compile(r"\d+")

# For the word pipeline we strip standard Latin punctuation plus a few
# typical Wikipedia characters. We replace with space (never delete)
