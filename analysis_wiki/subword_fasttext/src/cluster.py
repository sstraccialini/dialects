...
Identical logic to the TF-IDF baseline; reused here so the approach
is self-contained.
"""
...
def silhouette_report(dist: np.ndarray, codes: List[str]) -> dict:
    ...
    return {
        "silhouette_family": float(silhouette_score(dist, family, metric="precomputed")),
        "silhouette_romance_vs_rest": float(silhouette_score(dist, romance, metric="precomputed")),
...
def hierarchical_linkage(dist: np.ndarray, method: str = "average") -> np.ndarray:
...
def cluster_pipeline(
    ...
    """Silhouette + linkage + dendrogram for one pipeline."""
    ...
    plot_dendrogram(
        Z, codes,
        title=f"Hierarchical clustering ({pipeline}, cosine + average linkage)",
...
def save_silhouette_report(reports: List[dict]) -> Path:
    ...
    lines += [
        "Notes:",
        "  - family labels: italo_romance, italian, romance, germanic, english,",
        "    greek, semitic, slavic (from config.VARIETIES).",
        "  - romance vs rest: binary, romance = {italo_romance, italian, romance}.",
        "  - Silhouette range: [-1, 1]. >0.2 good, >0.5 excellent, ~0 no structure.",
    ]
